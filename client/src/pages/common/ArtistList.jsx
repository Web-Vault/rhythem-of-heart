    ...
    <>
      <section className="artists-list-section">
        ...
        <div className="artists-list-grid grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 max-w-6xl mx-auto px-4 md:px-0">
          {artists.map((artist) => (
            <div key={artist.id} className="artist-card group relative overflow-hidden flex flex-col items-center p-0 border border-[#e0e7ff] rounded-2xl bg-gradient-to-br from-white via-indigo-50 to-indigo-100 shadow-md hover:shadow-xl transition-all">
              <div className="artist-accent-bar"></div>
              <div className="flex flex-col items-center w-full pt-8 pb-4 px-7">
                <img src={artist.photo} alt={artist.name} className="artist-photo mb-5 rounded-full border-4 border-indigo-100 w-28 h-28 object-cover shadow" />
                <h3 className="artist-name text-xl font-extrabold text-indigo-800 mb-2 text-center">{artist.name}</h3>
                <p className="artist-bio text-gray-700 text-center mb-4 px-2">{artist.bio}</p>
              ...
              <Link to={`/artist/${artist.id}`} className="artist-tile-details-btn-wrap w-full px-7 pb-7">
